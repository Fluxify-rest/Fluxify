...
const InstanceSettingsTab = () => {
	...
	const authCfg =
		(publicSettings?.auth_config as Json | undefined) ?? undefined;
		...
		<Container size="md" py="xl">
			<Stack gap="lg">
							...
							<>
								...
								<Group grow>
									<TextInput
										...
										onChange={(e) =>
											setDiscoveryEndpoint(e.currentTarget.value)
										}
									/>
									<TextInput
						...
						<Group grow align="flex-end">
							<Select
								...
								data={[
									{
										value: "traditional",
										label: "traditional (email + password)",
									},
				...
				<Accordion
					variant="separated"
					multiple
					defaultValue={["idp", "redirect"]}
				>
					<Accordion.Item value="idp">
						<Accordion.Control>
							1. In your identity provider (IdP)
						</Accordion.Control>
						<Accordion.Panel>
							<Text size="sm" mb="xs">
								Create an application in your IdP (Okta, Entra ID / Azure AD,
								Auth0, Keycloak, Google Workspace, etc.). Pick <b>OIDC</b> for
								the simplest setup, or <b>SAML 2.0</b> if your org requires it.
								You will collect:
							</Text>
							<List size="sm" spacing={4}>
								<List.Item>
									<b>Issuer URL</b> — the provider&apos;s base URL. In OIDC
									it&apos;s the value under <Code>issuer</Code> in the
									provider&apos;s <Code>/.well-known/openid-configuration</Code>{" "}
									document (e.g. Okta: <Code>https://your-org.okta.com</Code>;
									Entra:{" "}
									<Code>
								...
								<List.Item>
									<b>Client ID</b> &amp; <b>Client Secret</b> — generated when
									you register the app. Found on the app&apos;s
								...
								<List.Item>
									<b>Sign-on URL / SSO URL</b> (=<Code>entryPoint</Code>) and
									the <b>X.509 signing certificate</b> (=<Code>samlCert</Code>)
									— from the IdP app&apos;s SAML settings / metadata. (SAML)
								</List.Item>
								<List.Item>
									<b>Email domain</b> your users sign in with (e.g.{" "}
									<Code>company.com</Code>). This becomes <Code>domain</Code>{" "}
									and gates which accounts an admin may pre-provision.
								</List.Item>
							</List>
						</Accordion.Panel>
					</Accordion.Item>
					...
					<Accordion.Item value="provision">
						...
						<Accordion.Panel>
							<Text size="sm" mb="xs">
								In <b>sso_only</b> mode, creating a user here only registers
								them in the platform (email must match the{" "}
								<Code>sso_config.domain</Code>
								); their Better Auth login account is created automatically the
								first time they sign in through the IdP. In <b>
									traditional
								</b>{" "}
...
export default InstanceSettingsTab;
