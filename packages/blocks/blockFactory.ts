...
import { CustomBlock } from "./builtin/customBlock";
...
export class BlockFactory {
	constructor(
		...
		private readonly customBlockGetter?: (type: string) => any,
	) {}
	...
	public createBlock(
		...
		switch (block.type) {
			case BlockTypes.entrypoint:
				return new EntrypointBlock(
					this.context,
					block.data,
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.if:
				return this.createIfBlock(block, edgesMap);
			case BlockTypes.forloop:
				return this.createForLoopBlock(block, builder, edgesMap);
			case BlockTypes.foreachloop:
				return this.createForEachLoopBlock(block, builder, edgesMap);
			case BlockTypes.transformer:
				return new TransformerBlock(
					this.context,
					this.validate(transformerBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.setvar:
				return new SetVarBlock(
					this.context,
					this.validate(setVarSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.getvar:
				return new GetVarBlock(
					this.context,
					this.validate(getVarBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.consolelog:
				return new ConsoleLoggerBlock(
					this.context,
					block.data,
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.jsrunner:
				return new JsRunnerBlock(
					this.context,
					this.validate(jsRunnerBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.response:
				return new ResponseBlock(
					this.context,
					this.validate(responseBlockSchema, block.data),
				);
			case BlockTypes.arrayops:
				return new ArrayOperationsBlock(
					this.context,
					this.validate(arrayOperationsBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpGetHeader:
				return new GetHttpHeaderBlock(
					this.context,
					this.validate(getHttpHeaderBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpSetHeader:
				return new SetHttpHeaderBlock(
					this.context,
					this.validate(getHttpHeaderBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpGetParam:
				return new GetHttpParamBlock(
					this.context,
					this.validate(getHttpParamBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpGetCookie:
				return new GetHttpCookieBlock(
					this.context,
					this.validate(getHttpCookieBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpSetCookie:
				return new SetHttpCookieBlock(
					this.context,
					this.validate(setHttpCookieBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httpGetRequestBody:
				return new GetHttpRequestBodyBlock(
					this.context,
					null,
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.httprequest:
				return new HttpRequestBlock(
					this.context,
					this.validate(httpRequestBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_getsingle:
				return new GetSingleDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(getSingleDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_getall:
				return new GetAllDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(getAllDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_insert:
				return new InsertDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(insertDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_delete:
				return new DeleteDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(deleteDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_insertbulk:
				return new InsertBulkDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(insertBulkDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_update:
				return new UpdateDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(updateDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_native:
				return new NativeDbBlock(
					this.context,
					this.getDbAdapter(block.data.connection),
					this.validate(nativeDbBlockSchema, block.data),
					this.findEdge(block, "source", edgesMap),
				);
			case BlockTypes.db_transaction:
				return this.createDbTransactionBlock(block, builder, edgesMap);
			case BlockTypes.errorHandler:
				return this.createErrorHandlerBlock(block, edgesMap);
			case BlockTypes.cloudLogs:
				return this.createCloudLogsBlock(block, edgesMap);
			default: {
				if (this.customBlockGetter) {
					const customGraphs = this.customBlockGetter(block.type);
					if (
						customGraphs &&
						Array.isArray(customGraphs) &&
	...
	private findEdge(
		...
		return (
			edgesMap[block.id]?.find((edge) => edge.handle == handleType)?.to || ""
		);
	}
	...
	private createIfBlock(block: BlockDTOType, edgesMap: EdgesType) {
		...
		return new IfBlock(
			this.findEdge(block, "success", edgesMap),
			this.findEdge(block, "failure", edgesMap),
			this.context,
			data,
		);
	}

	private createForLoopBlock(
		block: BlockDTOType,
		builder: any,
		edgesMap: EdgesType,
	) {
		...
		return new ForLoopBlock(
			this.context,
			{ ...data, block: executor },
			this.engineFactory.create(builder, executor),
			this.findEdge(block, "source", edgesMap),
		);
	}

	private createForEachLoopBlock(
		block: BlockDTOType,
		builder: any,
		edgesMap: EdgesType,
	) {
	...
	private createDbTransactionBlock(
		block: BlockDTOType,
		builder: any,
		edgesMap: EdgesType,
	) {
		...
		return new TransactionBlock(
			this.context,
			this.getDbAdapter(data.connection),
			data,
			this.engineFactory.create(builder, executor),
			this.findEdge(block, "source", edgesMap),
		);
	...
	private createCloudLogsBlock(block: BlockDTOType, edgesMap: EdgesType) {
		...
		const logger = this.integrationFactory.create({
			integrationId: data.connection,
			type: "observability",
		});
		return new CloudLogsBlock(
			this.context,
			logger,
			data,
			this.findEdge(block, "source", edgesMap),
	...
	private createCustomBlock(
		...
		const edges: ReturnType<
			typeof import("./builderTypes").edgeDTOSchema.parse
		> = [];
		...
		for (const graph of customGraphs) {
			...
			const resolveParams = (obj: any) => {
				...
				for (const key in obj) {
					const val = obj[key];
					if (typeof val === "string" && val.startsWith("param:")) {
						const paramName = val.slice(6);
						if (block.data && block.data[paramName] !== undefined) {
							obj[key] = block.data[paramName];
						}
			...
			for (const conn of connections) {
				edges.push({
					...
					from: graph.id,
					to: conn.to,
					fromHandle: conn.toHandle,
					toHandle: conn.fromHandle,
				});
			}

			// We don't want 'connections' confusing the sub-block execution
		...
		const cbBuilder = new BlockBuilder(
			this.context,
			this.engineFactory,
			this.integrationFactory,
			this.shouldValidateBlockData,
			this.customBlockGetter,
		);
		...
		if (!executor) {
			throw new Error(
				`Custom block '${block.type}' does not have an entrypoint block.`,
			);
		}

		const subEngine = this.engineFactory.create(cbBuilder, executor);
